...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Function to show a specific view -> Removed from here to be global
    ...
    const translations = {
        en: {
            ...
            projectTitle: "GUTANGLE: A Smart Belt for Spine and Stomach Wellness",
    ...
    // 7. FORM HANDLING
    // ============================================
    // Note: Actual form submission is handled by Flask (POST request)
    // We only need client-side validation here if we want to intercept it.
    // The previous code was preventing default and just showing an alert.
    // We will let the form submit naturally if captcha passes.

    // The event listener in setupCaptcha handles the submission logic now.
    // It calls handleSignup/Signin if captcha matches.
    ...
    // Check if we are in SPA mode (all views exist)
    const hasLanding = document.getElementById('view-landing');
    const hasLogin = document.getElementById('view-login');
    const hasDashboard = document.getElementById('view-dashboard');

    if (hasLanding && hasLogin && hasDashboard) {
        // SPA Mode
        ...
        // MPA Mode - Ensure the current page's view is visible
        // The individual pages (login.html, dashboard.html) should have their views visible by default,
        // but just in case showView was called or styles interfered:
        if (hasLogin) {
            hasLogin.style.display = 'block';
            window.dispatchEvent(new Event('resize')); // For canvas
        }
        if (hasDashboard) {
            hasDashboard.style.display = 'block';
            if (typeof initDashboard === 'function') {
                setTimeout(initDashboard, 100);
            }
        }
        // Landing is usually visible by default in its template
    }
    ...
    if (bypassLoginBtn) {
        bypassLoginBtn.addEventListener('click', () => {
            ...
            localStorage.setItem('userEmail', 'demo@gutangle.com');
