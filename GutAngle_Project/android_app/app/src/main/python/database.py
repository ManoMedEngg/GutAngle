...
# Writable directory for database on Android
if "FILESDIR" in os.environ:
    db_dir = os.environ["FILESDIR"]
else:
    # Fallback to home dir which is typically writable in Chaquopy
...
print(f"DATABASE: Resolved path to: {DATABASE}")

# Ensure the database exists or copy it from assets if it doesn't
if not os.path.exists(DATABASE):
    asset_db_path = join(dirname(__file__), 'gutangle.db')
    if os.path.exists(asset_db_path):
        print(f"DATABASE: Copying initial DB from {asset_db_path}")
        shutil.copy2(asset_db_path, DATABASE)
    else:
        print("DATABASE: Asset DB not found, creating new empty database.")
