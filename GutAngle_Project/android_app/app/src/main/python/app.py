...
def load_sim_datasets():
    """Load EGG and Spinal datasets for simulation"""
    try:
        base_dir = os.path.dirname(__file__)
        # 1. Load EGG Data (Stomach)
        egg_path = os.path.join(base_dir, 'Dataset/EGG_dataset/EGG-database/ID4_postprandial.txt')
        if os.path.exists(egg_path):
            with open(egg_path, 'r') as f:
                for line in f:
                    parts = line.split()
                    if parts:
                        SIM_DATA['egg'].append(float(parts[0]))
        
        # 2. Load Spinal Data (Flex, Pitch, Roll, Yaw)
        spinal_path = os.path.join(base_dir, 'Dataset/Spainal_dataset/gutANGLE - Mano_Spinal_Cord_Data.csv')
        if os.path.exists(spinal_path):
            import csv
            with open(spinal_path, 'r') as f:
                # Skip to line 7 (data start)
                for _ in range(6): next(f, None)
                reader = csv.reader(f)
                count = 0
                for row in reader:
                    if len(row) > 80:
                        # Extract: Flex(75), Pitch(79), Roll(80), Yaw(78)
                        # csv indices are 0-based, so 74, 78, 79, 77
                        SIM_DATA['spinal'].append((
                            float(row[74] or 0), # Flex
                            float(row[78] or 0), # Pitch
                            float(row[79] or 0), # Roll
                            float(row[77] or 0)  # Yaw
                        ))
                        count += 1
                        if count >= 72000: break # Exactly 1 hour
        print(f"FLASK: Loaded {len(SIM_DATA['egg'])} EGG points and {len(SIM_DATA['spinal'])} Spinal points for simulation.")
    except Exception as e:
        print(f"FLASK: Error loading datasets: {e}")

# Load once at start
load_sim_datasets()

# init_db() - Moved to start_server()
...
def dashboard():
    ...
    if request.args.get('simulation') == 'true':
        ...
        SIM_DATA['start_time'] = time.time()
        print("FLASK: Simulation mode activated.")
    else:
        # Keep existing simulation state if already set, but don't force it
        pass
...
@app.route('/api/simulation/enable', methods=['POST'])
def enable_simulation():
    """Explicitly enable simulation mode"""
    session['simulation'] = True
    SIM_DATA['start_time'] = time.time()
    print("FLASK: Simulation mode explicitly activated via API.")
    return jsonify({'success': True})
    ...
    def generate():
        ...
        while True:
            try:
                ...
                line = belt_data_queue.get(timeout=0.05)
                yield f'data: {line}\n\n'
            except queue.Empty:
                # If no real data, check if simulation is active
                if session.get('simulation') and SIM_DATA['spinal']:
                    now = time.time()
                    # Ensure 50ms interval (20Hz)
                    if now - last_sim_time >= 0.05:
                        elapsed = now - SIM_DATA['start_time'] if SIM_DATA['start_time'] else 0
                        idx = int(elapsed * 20) % 72000 # 1-hour loop
                        
                        # Get Spinal Data
                        flex, pitch, roll, yaw = SIM_DATA['spinal'][idx % len(SIM_DATA['spinal'])]
                        
                        # Get EGG Data (loop every 2400 points)
                        stomach = 0
                        if SIM_DATA['egg']:
                            stomach = SIM_DATA['egg'][idx % len(SIM_DATA['egg'])]
                        
                        # Placeholder BPM
                        bpm = 72 + (math.sin(elapsed / 10) * 5)
                        
                        # Format: Stomach, Flex, Pitch, Roll, Yaw, BPM
                        line = f"{stomach:.3f},{flex:.2f},{pitch:.2f},{roll:.2f},{yaw:.2f},{bpm:.0f}"
                        last_sim_time = now
                        yield f'data: {line}\n\n'
                    else:
                        time.sleep(0.01)
...
def start_server():
    import socket
    # Simple check to see if port 5000 is already in use
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        if s.connect_ex(('127.0.0.1', 5000)) == 0:
            print("FLASK: Server already running on port 5000, skipping start_server.")
            return

    try:
        print("FLASK: Starting server initialization...")
        init_db()
        print("FLASK: Database initialized successfully!")
        # Use host '0.0.0.0' for better accessibility and disable debug/reloader
        app.run(host='0.0.0.0', port=5000, threaded=True, debug=False)
    except BaseException as e:
        error_msg = f"FLASK_CRASH: {str(e)}"
        print(error_msg)
        import traceback
        traceback.print_exc()
        
        # Try to report back to Android UI if possible
        try:
            from com.chaquo.python import Python
            # In MainActivity we added logPythonError to the WebAppInterface
            # But we can also just log it to Logcat which we're already doing.
            pass
        except:
            pass
